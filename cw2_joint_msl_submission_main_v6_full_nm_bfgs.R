# =========================================================
# Coursework 2: submission-ready joint MSL (brief-aligned v6)
# =========================================================
# Main design choices:
...
#   - Bargaining equation includes an intercept so the latent factor location
#     is not forced to imply E[s_m_input] = 0 at the anchor.
...
#   - Halton draws only.
#   - Main optimisation uses larger-iteration Nelder-Mead then BFGS.
...
#   - The participation/wage sample is aligned by defining workers only
...
fit_joint_msl_brief_v6 <- function(data_path = "prepdata.csv",
                                   include_lr_inc_mu = FALSE,
                                   use_logit_sm = TRUE,
                                   anchor_indicator = "s_m_input",
                                   R = 500,
  ...
  cat("\n==============================\n")
  cat("Joint MSL brief-aligned v6 run\n")
  cat("==============================\n")
  ...
  cat("--- Nelder-Mead finished ---\n")
  cat("NM convergence code =", nm_fit$convergence, "\n")
  cat("NM objective =", nm_fit$value, "\n")
  ...
  cat("--- BFGS finished ---\n")
  cat("BFGS convergence code =", bfgs_fit$convergence, "\n")
  cat("BFGS objective =", bfgs_fit$value, "\n")

  grad_norm <- NA_real_
  if (isTRUE(compute_grad_norm) && requireNamespace("numDeriv", quietly = TRUE)) {
    cat("\n--- Computing final gradient norm ---\n")
    grad_try <- try(
      objective_grad_v2(bfgs_fit$par, prepped = prepped, draws = draws),
      silent = TRUE
    )
    if (!inherits(grad_try, "try-error")) {
      grad_norm <- sqrt(sum(grad_try^2))
      cat("Final gradient norm =", grad_norm, "\n")
    } else {
      cat("Gradient norm could not be computed.\n")
    }
  }
...
compute_hessian_joint_msl_v6 <- function(object,
                                         hessian_method = c("optimHess", "numDeriv")) {
  if (!inherits(object, "cw2_joint_msl_v6")) stop("object must inherit from 'cw2_joint_msl_v6'.")
...
compute_inference_joint_msl_v6 <- function(object,
                                           hessian_method = c("optimHess", "numDeriv"),
                                           use_robust = TRUE,
                                           eig_floor_abs = 1e-6,
                                           eig_floor_rel = 1e-8,
  ...
  H <- compute_hessian_joint_msl_v6(object, hessian_method = hessian_method)
...
tidy_joint_msl_v6 <- function(object, transformed = FALSE, use_robust = TRUE) {
  if (!inherits(object, "cw2_joint_msl_v6")) stop("object must inherit from 'cw2_joint_msl_v6'.")
...
summarize_joint_msl_v6 <- function(object) {
  if (!inherits(object, "cw2_joint_msl_v6")) stop("object must inherit from 'cw2_joint_msl_v6'.")
  ...
  list(
    optimization = tibble(
      nm_convergence = object$nm_fit$convergence,
      bfgs_convergence = object$fit$convergence,
      optimizer_message = case_when(
        object$fit$convergence == 0 ~ "BFGS converged",
        object$fit$convergence == 1 ~ "BFGS reached iteration limit",
        TRUE ~ paste0("BFGS convergence code ", object$fit$convergence)
      ),
      nm_neg_loglik = object$nm_fit$value,
...
posterior_mu_mean_v6 <- function(object) {
  if (!inherits(object, "cw2_joint_msl_v6")) stop("object must inherit from 'cw2_joint_msl_v6'.")
  dims <- make_dim_list(object$prepped)
  p <- unpack_theta_v2(object$fit$par, dims)
...
append_posterior_mu_v6 <- function(object) {
  object$prepped$joint_df %>% mutate(mu_post = posterior_mu_mean_v6(object))
}

extract_best_theta_v6 <- function(result_object) {
  if (!is.list(result_object) || is.null(result_object$fit) || is.null(result_object$fit$fit$par)) {
    stop("result_object must be the list returned by run_cw2_submission_main_v6().")
  }
  result_object$fit$fit$par
}

save_best_theta_v6 <- function(result_object, path = "cw2_best_theta_submission_v6.rds") {
  out <- list(
    theta = extract_best_theta_v6(result_object),
...
build_submission_output_v6 <- function(fit_object, use_robust = TRUE) {
  list(
    fit = fit_object,
    summary = summarize_joint_msl_v6(fit_object),
    coefficients_raw = tidy_joint_msl_v6(fit_object, transformed = FALSE, use_robust = FALSE),
    coefficients_structural = tidy_joint_msl_v6(fit_object, transformed = TRUE, use_robust = use_robust),
    bargaining_index = append_posterior_mu_v6(fit_object)
  )
}
...
run_cw2_submission_main_v6 <- function(data_path = "prepdata.csv",
                                       theta_start_path = NULL,
                                       include_lr_inc_mu = FALSE,
                                       use_logit_sm = TRUE,
                                       anchor_indicator = "s_m_input",
  ...
  cat("\n--- Computing Hessian and inference ---\n")
  fit1 <- compute_inference_joint_msl_v6(
    fit0,
  ...
  cat("--- Inference finished ---\n")

  build_submission_output_v6(fit1, use_robust = use_robust)
}
...
# main_submission_v6 <- run_cw2_submission_main_v6(
...
# main_submission_v6 <- run_cw2_submission_main_v6(
#   data_path = "prepdata.csv",
#   theta_start_path = "cw2_best_theta_submission_v6.rds",
#   include_lr_inc_mu = FALSE,
#   use_logit_sm = TRUE,
#   anchor_indicator = "s_m_input",
#   R = 500,
...
#
# Print core outputs:
# print(main_submission_v6$summary$optimization)
# print(main_submission_v6$summary$accounting, n = Inf)
# print(main_submission_v6$summary$hessian_diagnostics)
# print(main_submission_v6$summary$robust_diagnostics)
# print(main_submission_v6$summary$measurement, n = Inf)
# print(main_submission_v6$summary$participation, n = Inf)
# print(main_submission_v6$summary$wage, n = Inf)
# print(main_submission_v6$summary$selection_scales, n = Inf)
# print(main_submission_v6$coefficients_structural, n = Inf)
# print(cor(main_submission_v6$bargaining_index$mu_post,
#           main_submission_v6$bargaining_index$s_m,
#           use = "complete.obs"))
# print(summary(main_submission_v6$bargaining_index$mu_post))
#
# Save outputs:
# saveRDS(main_submission_v6, "cw2_submission_main_v6_results.rds")
# write.csv(main_submission_v6$coefficients_structural,
#           "cw2_submission_main_v6_coefficients.csv",
#           row.names = FALSE)
# write.csv(main_submission_v6$bargaining_index,
#           "cw2_submission_main_v6_bargaining_index.csv",
#           row.names = FALSE)
#
# Save theta for fast same-spec reruns:
# save_best_theta_v6(main_submission_v6, "cw2_best_theta_submission_v6.rds")
