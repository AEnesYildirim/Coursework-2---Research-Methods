...
# Strong-indicator re-anchored version with richer wage equation:
#   W_f ~ age.f + age_sq + educ.f + educ_sq + age_educ
# Default anchor is yieldfor.proj.m
# =========================================================
...
build_joint_sample <- function(data_path = "prepdata.csv",
  ...
  joint_df <- df %>%
    select(
      ...
      age.f, educ.f, age_sq, educ_sq, age_educ,
      D_f, W_f
    ) %>%
    filter(
      if_all(
        c(base_y_names,
          ...
          "age.f", "educ.f", "age_sq", "educ_sq", "age_educ", "D_f"),
        ~ !is.na(.)
      )
    ) %>%
  ...
  X_w_df <- joint_df %>% select(age.f, age_sq, educ.f, educ_sq, age_educ)
  ...
  list(
    ...
    settings = list(
      ...
      wage_spec = "age.f + age_sq + educ.f + educ_sq + age_educ"
    )
  )
}
...
get_initial_values_joint <- function(prepped) {
  ...
  # Wage starts from worker-only OLS with richer specification
  wage_fit <- lm(
    W_f ~ age.f + age_sq + educ.f + educ_sq + age_educ,
  ...
  if (requireNamespace("sampleSelection", quietly = TRUE)) {
    heckman_try <- try(
      sampleSelection::selection(
        ...
        outcome   = W_f ~ age.f + age_sq + educ.f + educ_sq + age_educ,
        data = init_df,
        method = "ml"
      ),
      silent = TRUE
...
fit_joint_msl <- function(data_path = "prepdata.csv",
  ...
  cat("Wage spec = age.f + age_sq + educ.f + educ_sq + age_educ\n")
  cat("R draws =", R, "\n")
