...
# Centered-polynomial wage version: age and education are centered before
# squaring and interacting in the wage equation.
# =========================================================
...
make_centered_wage_terms <- function(df, age_center = NULL, educ_center = NULL) {
  if (is.null(age_center)) age_center <- mean(df$age.f, na.rm = TRUE)
  if (is.null(educ_center)) educ_center <- mean(df$educ.f, na.rm = TRUE)

  out <- df %>%
    mutate(
      age_c = age.f - age_center,
      educ_c = educ.f - educ_center,
      age_c_sq = age_c^2,
      educ_c_sq = educ_c^2,
      agec_educc = age_c * educ_c
    )

  attr(out, "age_center") <- age_center
  attr(out, "educ_center") <- educ_center
  out
}
...
build_joint_sample <- function(data_path = "prepdata.csv",
  ...
  X_w_df <- make_centered_wage_terms(joint_df %>% select(age.f, educ.f))
  age_center <- attr(X_w_df, "age_center")
  educ_center <- attr(X_w_df, "educ_center")
  X_w <- model.matrix(~ age_c + age_c_sq + educ_c + educ_c_sq + agec_educc, data = X_w_df)
...
get_initial_values_joint <- function(prepped) {
  ...
  # Measurement starts from the current factor-analysis workflow
  fa_start <- factanal(
    x = prepped$Y,
    factors = 1,
    scores = "regression"
  ...
  init_df <- prepped$joint_df %>%
    mutate(
      mu0 = mu0,
      age_c = age.f - prepped$settings$age_center,
      educ_c = educ.f - prepped$settings$educ_center,
      age_c_sq = age_c^2,
      educ_c_sq = educ_c^2,
      agec_educc = age_c * educ_c
    )
  ...
  wage_fit <- lm(
    W_f ~ age_c + age_c_sq + educ_c + educ_c_sq + agec_educc,
  ...
  if (requireNamespace("sampleSelection", quietly = TRUE)) {
    heckman_try <- try(
      sampleSelection::selection(
        ...
        outcome   = W_f ~ age_c + age_c_sq + educ_c + educ_c_sq + agec_educc,
        data = init_df,
        method = "ml"
      ),
      silent = TRUE
