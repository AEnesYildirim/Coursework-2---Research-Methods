# =========================================================
# Coursework 2: submission-ready joint MSL (brief-aligned v2)
# =========================================================
# Main design choices:
...
#   - Halton draws only.
#   - One optimisation path only: Nelder-Mead then BFGS.
...
build_joint_sample_v2 <- function(data_path = "prepdata.csv",
  ...
  df <- df %>%
    mutate(
      ...
      D_f = case_when(
        is.na(income.f) & is.na(hours.f) ~ NA_integer_,
        (!is.na(income.f) & income.f > 0) | (!is.na(hours.f) & hours.f > 0) ~ 1L,
        TRUE ~ 0L
      ),
      wage_f = case_when(
        !is.na(income.f) & !is.na(hours.f) & income.f > 0 & hours.f > 0 ~ income.f / hours.f,
        TRUE ~ NA_real_
      ),
      W_f = case_when(
  ...
  X_mu <- model.matrix(~ . - 1, data = X_mu_std$data)   # NO intercept, as in the brief

  Z_D_df <- joint_df %>% select(all_of(vars$z_vars_all))
  Z_D <- model.matrix(~ ., data = Z_D_df)               # intercept included automatically

  X_w_df <- joint_df %>% select(all_of(c("age.f", "educ.f")))
  X_w <- model.matrix(~ ., data = X_w_df)               # intercept included automatically
...
unpack_theta_v2 <- function(theta, dims) {
  ...
  list(
    beta = c(0, beta_free),           # anchor intercept fixed to 0
    lambda = c(1, lambda_free),       # anchor loading fixed to 1
...
get_initial_values_v2 <- function(prepped) {
...
fit_joint_msl_brief_v2 <- function(data_path = "prepdata.csv",
                                   include_lr_inc_mu = FALSE,
                                   use_logit_sm = TRUE,
                                   anchor_indicator = "s_m_input",
                                   R = 500,
...
compute_hessian_joint_msl_v2 <- function(object,
                                         hessian_method = c("optimHess", "numDeriv")) {
  if (!inherits(object, "cw2_joint_msl_v2")) stop("object must inherit from 'cw2_joint_msl_v2'.")
...
compute_inference_joint_msl_v2 <- function(object,
                                           hessian_method = c("optimHess", "numDeriv"),
                                           use_robust = TRUE,
                                           eig_floor_abs = 1e-6,
                                           eig_floor_rel = 1e-8,
  ...
  H <- compute_hessian_joint_msl_v2(object, hessian_method = hessian_method)
...
tidy_joint_msl_v2 <- function(object, transformed = FALSE, use_robust = TRUE) {
  if (!inherits(object, "cw2_joint_msl_v2")) stop("object must inherit from 'cw2_joint_msl_v2'.")
...
summarize_joint_msl_v2 <- function(object) {
  if (!inherits(object, "cw2_joint_msl_v2")) stop("object must inherit from 'cw2_joint_msl_v2'.")
...
posterior_mu_mean_v2 <- function(object) {
  if (!inherits(object, "cw2_joint_msl_v2")) stop("object must inherit from 'cw2_joint_msl_v2'.")
  dims <- make_dim_list(object$prepped)
  p <- unpack_theta_v2(object$fit$par, dims)
...
append_posterior_mu_v2 <- function(object) {
  object$prepped$joint_df %>% mutate(mu_post = posterior_mu_mean_v2(object))
}

extract_best_theta_v2 <- function(result_object) {
  if (!is.list(result_object) || is.null(result_object$fit) || is.null(result_object$fit$fit$par)) {
    stop("result_object must be the list returned by run_cw2_submission_main_v2().")
  }
  result_object$fit$fit$par
}

save_best_theta_v2 <- function(result_object, path = "cw2_best_theta_submission_v2.rds") {
  out <- list(
    theta = extract_best_theta_v2(result_object),
...
build_submission_output_v2 <- function(fit_object, use_robust = TRUE) {
  list(
    fit = fit_object,
    summary = summarize_joint_msl_v2(fit_object),
    coefficients_raw = tidy_joint_msl_v2(fit_object, transformed = FALSE, use_robust = FALSE),
    coefficients_structural = tidy_joint_msl_v2(fit_object, transformed = TRUE, use_robust = use_robust),
    bargaining_index = append_posterior_mu_v2(fit_object)
  )
}
...
run_cw2_submission_main_v2 <- function(data_path = "prepdata.csv",
                                       theta_start_path = NULL,
                                       include_lr_inc_mu = FALSE,
                                       use_logit_sm = TRUE,
                                       anchor_indicator = "s_m_input",
  ...
  fit1 <- compute_inference_joint_msl_v2(
    fit0,
  ...
  build_submission_output_v2(fit1, use_robust = use_robust)
}
...
# main_submission_v2 <- run_cw2_submission_main_v2(
...
# main_submission_v2 <- run_cw2_submission_main_v2(
#   data_path = "prepdata.csv",
#   theta_start_path = "cw2_best_theta_submission_v2.rds",
#   include_lr_inc_mu = FALSE,
#   use_logit_sm = TRUE,
#   anchor_indicator = "s_m_input",
#   R = 500,
...
#
# Print core outputs:
# print(main_submission_v2$summary$optimization)
# print(main_submission_v2$summary$accounting, n = Inf)
# print(main_submission_v2$summary$hessian_diagnostics)
# print(main_submission_v2$summary$robust_diagnostics)
# print(main_submission_v2$summary$measurement, n = Inf)
# print(main_submission_v2$summary$participation, n = Inf)
# print(main_submission_v2$summary$wage, n = Inf)
# print(main_submission_v2$summary$selection_scales, n = Inf)
# print(main_submission_v2$coefficients_structural, n = Inf)
# print(cor(main_submission_v2$bargaining_index$mu_post,
#           main_submission_v2$bargaining_index$s_m,
#           use = "complete.obs"))
# print(summary(main_submission_v2$bargaining_index$mu_post))
#
# Save outputs:
# saveRDS(main_submission_v2, "cw2_submission_main_v2_results.rds")
# write.csv(main_submission_v2$coefficients_structural,
#           "cw2_submission_main_v2_coefficients.csv",
#           row.names = FALSE)
# write.csv(main_submission_v2$bargaining_index,
#           "cw2_submission_main_v2_bargaining_index.csv",
#           row.names = FALSE)
#
# Save theta for fast same-spec reruns:
# save_best_theta_v2(main_submission_v2, "cw2_best_theta_submission_v2.rds")
